...
def create_post(request):
    ...
    if request.method == 'POST':
        ...
        if post_form.is_valid():
            ...
            words = content.split(' ') # 띄어쓰기 기준으로 split한다
            for word in words:
                if word[0] == '#':
                    word = word[1:]
                    tag = HashTag.objects.get_or_create(content=word) # (HashTagObj, False)
                    post.tags.add(tag[0])
                    if tag[1]: # tag가 처음 만들어 진거라면, 메세지 추가
    ...
    if post.user == request.user:
        if request.method == 'POST':
            ...
            if post_form.is_valid():
                ...
                #update HashTag
                post.tags.clear() # 기존의 post에 달린 tag 다 삭제하고
